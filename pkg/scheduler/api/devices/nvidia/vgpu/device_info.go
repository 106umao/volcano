...
func (gs *GPUDevices) Release(kubeClient kubernetes.Interface, pod *v1.Pod) (*devices.DeviceReservation, error) {
	...
	return &devices.DeviceReservation{
		...
		Annotations: annotationKeys(keys),
	}, nil
}
...
func (gs *GPUDevices) Allocate(kubeClient kubernetes.Interface, pod *v1.Pod) (*devices.DeviceReservation, error) {
	...
	// Persist allocation annotations for vgpu-device-plugin on the kubelet side.
	if err = patchPodAnnotations(kubeClient, pod, annotations); err != nil {
		return nil, err
	}
	klog.V(3).Infoln("DeviceSharing:Allocate Success")

	// Return allocation annotations so predicates can carry the same values at bind time.
	// Bind carry is an upsert, so this is idempotent with the patch above.
	return &devices.DeviceReservation{
		DeviceType:  DeviceName,
...
func annotationKeys(keys []string) map[string]string {
